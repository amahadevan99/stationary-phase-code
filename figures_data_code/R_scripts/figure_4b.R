...
# Evolution condition code mapping & order (10 day last)
...
# Only keep stats for panels in use
...
p <- ggplot(df, aes(
    ...
    geom_segment(
        aes(
            x    = .data[[X_VAR]] - .data[[X_ERR_VAR]],
            xend = .data[[X_VAR]] + .data[[X_ERR_VAR]],
            y    = .data[[Y_VAR]],
            yend = .data[[Y_VAR]]
        ),
        linewidth = 0.6
    ...
    geom_segment(
        aes(
            x    = .data[[X_VAR]],
            xend = .data[[X_VAR]],
            y    = .data[[Y_VAR]] - .data[[Y_ERR_VAR]],
            yend = .data[[Y_VAR]] + .data[[Y_ERR_VAR]]
        ),
        linewidth = 0.6
    ) +
    geom_point(size = 3) +
    
    # --- MODIFIED: Both trendline layers now exclude the 'Gly/Eth 8 day' panel ---
    
    # Layer 1: The black outline (thicker)
    geom_smooth(
        data = . %>% filter(evol_cond_label != "Gly/Eth 8 day"), # Filter this layer
        method = "lm", 
        se = FALSE,
    ...
    # Layer 2: The colored trendline (thinner, drawn on top)
    geom_smooth(
        data = . %>% filter(evol_cond_label != "Gly/Eth 8 day"), # Also filter this layer
        method = "lm", 
        se = FALSE,
        aes(group = ancestor_label, color = ancestor_label),
        linewidth = 1.5
    ) +
    ...
    geom_text(
        ...
        inherit.aes = FALSE,
        hjust = 0, vjust = 1, size = 5, color = "black", show.legend = FALSE
    ) +
...
ggsave("figures/fig_4b.pdf", p, width = 12, height = 10)
