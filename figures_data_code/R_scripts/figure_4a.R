####new script for figure 4a### 121525 ####
library(ggplot2)
library(dplyr)
library(broom)
...
p <- ggplot(df, aes(x = .data[[X_VAR]], y = .data[[Y_VAR]], color = ancestor_label, shape = ploidy_consensus)) +
    ...
    geom_smooth(
        ...
        color = "black",         # This layer is black
        linewidth = 2.0,         # and slightly thicker
        show.legend = FALSE
    ) +
    # This is the original colored trendline, drawn on top of the black one
    geom_smooth(
        method = "lm", 
        se = FALSE,
        aes(group = ancestor_label), 
        linewidth = 1.5          # Thinner, so the black shows through
    ) +
    ...
    # --- MODIFIED: Updated axis labels as requested ---
    labs(
        color = "Ancestor",
...
ggsave("figures/fig_4a.pdf", p, width = 8, height = 6)
