...
library(dplyr)
library(broom)
df <- read.csv("../R_data/adaptive_haploid_SMF2.csv")
...
p <- ggplot(df, aes(x = .data[[X_VAR]], y = .data[[Y_VAR]], color = ancestor_label, shape = ploidy_consensus)) +
    ...
    # --- MODIFIED: y=0 line is removed, but x=0 line is kept ---
    # geom_hline(yintercept = 0, color = "grey", linetype = "dashed") + # This remains removed
    geom_vline(xintercept = 0, color = "grey", linetype = "dashed") +   # This is put back
    ...
    scale_shape_manual(
        name = "Ploidy",
        values = c("haploid" = 16, "diploid" = 17, "undetermined" = 4),
...
ggsave("figures/fig_S11.pdf", p, width = 8, height = 6)
